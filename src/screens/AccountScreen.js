...
const AccountScreen = () => {
    const [items,setItems] = useState([]) //once per page load
    const [loading,setLoading] = useState(false) //refresher
    const getDetails = async ()=>{
      const querySnap = await firestore().collection('ads')
      .where('uid','==',auth().currentUser.uid)   //get ads posted only by the current logged in user
      .get()
    ...
    return (
        <View style={{flex:1}}>
            ...
            <Button style={{backgroundColor:"#cc5500"}} mode="contained" onPress={() => auth().signOut()}>
                     Logout
             </Button>
